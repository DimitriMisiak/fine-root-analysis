...
#        self.dir = '/'.join((run_dir, self.run, self.detector, 'CalibratedData'))
...
#            if 'CalibratedData' in fname:
        ...
        # HACK
        if self.run == 'tg28l000':
            self.all = Guardian(self.roots[:11])
        else:
...
class Root_proto_simu(Root_proto, Root_trig):
    """ Simpler root class for trigger and noise data.
    """
    def __init__(self, file_path):

        Root_proto.__init__(self, file_path)
        Root_trig.__init__(self, file_path)


class Guardian_proto_simu(Guardian_proto, Guardian_trig):
    """ Simpler root class for trigger and noise data.
    """
    def __init__(self, root_list):

        Guardian_proto.__init__(self, root_list)
        Guardian_trig.__init__(self, root_list)


class Root_reader_simu(object):
    """ Analysis class.
    """
    ...
    def __init__(self,
                 ...
                 simu,
                 detector='RED80',
                 run_dir='/home/misiak/Data/data_run57_neutron/SimuCoinc'):
...
#        self.dir = '/'.join((run_dir, self.run, self.detector, 'CalibratedData'))
        ...
        for fn in os.listdir(self.dir):
            if self.simu in fn:
                fn_dir = self.dir + '/' + fn
                for fname in os.listdir(fn_dir):
                    if 'ProcessedData' in fname:
        #            if 'CalibratedData' in fname:
                        self.files.append(fn+'/'+fname)
...
#        print(self.files)
        ...
        for fname in tqdm(self.files):
            fpath = '/'.join((self.dir, fname))
            part_root = Root_proto_simu(fpath)
            (self.roots).append(part_root)

        # HACK
        if self.run == 'tg28l000':
            self.all = Guardian_proto_simu(self.roots[:11])
        else:
            self.all = Guardian_proto_simu(self.roots)
